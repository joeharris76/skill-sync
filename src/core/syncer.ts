...
import { writeProjectConfig } from "./config-generator.js";
...
export async function planSync(input: PlanSyncInput): Promise<SyncPlan> {
  ...
  for (const skill of resolvedSkills) {
    ...
    const installMode = effectiveInstallMode(manifest.installMode, manifest.overrides, skill.name);
...
export async function applySync(input: ApplySyncInput): Promise<ApplySyncResult> {
  ...
  // Generate skill-sync.config.yaml in each target directory using the single
  // canonical serializer (shared with operations and the verify gate).
  let wroteConfig = false;
  let configPath: string | undefined;

  if (config && Object.keys(config).length > 0) {
    for (const target of targets) {
      configPath = await writeProjectConfig(target, config);
      wroteConfig = true;
    }
  }
