...
/**
 * Finalize a sync plan: validate no conflicts and write skillsync.config.yaml.
 *
...
export async function applySync(input: ApplySyncInput): Promise<ApplySyncResult> {
  ...
  // Generate skillsync.config.yaml in each target directory
  ...
  if (config && Object.keys(config).length > 0) {
    for (const target of targets) {
      ...
      configPath = join(target, "skillsync.config.yaml");
      const configContent =
